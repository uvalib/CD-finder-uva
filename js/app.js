(function ($, Drupal, drupalSettings) {
 ...
 //           { name: "Shared File Services", matches: [1,2,4,5,6,7] }
    ...
    $.Mustache.add("question-checkbox-template",
        ...
        <a class='control-indicator collapsed' data-toggle='collapse' aria-expanded='false' id='accordion-{{id}}' aria-describedby='question-{{id}}' aria-controls='explanation-{{id}}' href='#explanation-{{id}}''>\
            <span class='sr-only'>more information</span>\
        </a>\
        </div>\
        <div class='explanation collapse' role='region' id='explanation-{{id}}' aria-labelledby='accordion-{{id}}''>{{{description}}}</div>\
...
// instead of labels, we have field_data[field]["label"]
        ...
        for (var i=0;i<sortedfields.length;i++) {
            ...
            help_text_counter++;
            help =  servicehelp.field_data[field].value ? "<a class='popup' aria-haspop='true' data-toggle='collapse' href='#help-"+help_text_counter+"'><span class='sr-only'>More information about "+servicelist[0].field_data[field].label+"</span><span class='fa fa-info-circle'></span></a><div class='help' id='help-"+help_text_counter+"'>"+servicehelp.field_data[field].value+"</div>" : "";
            chart = chart + help;
